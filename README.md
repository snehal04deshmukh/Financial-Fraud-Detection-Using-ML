...
Selecting relevant transaction features.

Separating input features (X) and target variable (y).

Converting the categorical transaction type into numerical form using one-hot encoding.

Applying a stratified train-test split to preserve the fraud/non-fraud class distribution.

Integrating preprocessing and model prediction into a machine learning pipeline.

Handling the highly imbalanced nature of the fraud dataset during model training and evaluation.
...
The user enters sender and receiver balance details.

The application preprocesses the input in the same format used during training.
